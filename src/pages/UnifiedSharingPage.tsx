...
const UnifiedSharingPage: React.FC = () => {
  ...
  const handleDocumentsUploaded = (documents: any[]) => {
    ...
    setPermissions(prev => ({
      ...prev,
      selectedDocuments: docs.map(doc => doc.id)
    }));
    ...
    <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="max-w-4xl mx-auto text-center">
      ...
      <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-6">
        {methods.map((method) => (
          ...
          >
            <div className={`w-16 h-16 bg-gradient-to-r ${method.color} rounded-2xl flex items-center justify-center text-3xl mb-6 mx-auto`}>
              {method.icon}
            </div>
    ...
    <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="max-w-2xl mx-auto">
      <div className="bg-white rounded-2xl shadow-xl p-8">
        <div className="text-center mb-8">
          <div className={`w-16 h-16 bg-gradient-to-r ${selectedMethod?.color} rounded-2xl flex items-center justify-center text-3xl mx-auto mb-4`}>
            {selectedMethod?.icon}
          </div>
